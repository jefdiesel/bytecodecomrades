...
/// @notice Minimal on-chain marketplace for ComradeRare ERC-721s, priced in COMRADE.
/// - Sellers list with `list(tokenId, price)` after approving the market for transfers.
/// - Buyers pay in COMRADE via `buy(tokenId)` after approving the market to spend.
...
contract ComradeMarket {
    IERC721  public immutable rareNft;
    IERC20Min public immutable comradeToken;
    ...
    constructor(IERC721 _rareNft, IERC20Min _comradeToken, address _feeRecipient, uint16 _feeBps) {
        if (_feeBps > 1000) revert FeeTooHigh();
        rareNft      = _rareNft;
        comradeToken   = _comradeToken;
        owner        = msg.sender;
    ...
    /// @notice List a ComradeRare for sale at a fixed COMRADE price.
    ...
    /// at least `price` COMRADE. The rare is transferred from seller to buyer.
    function buy(uint256 tokenId) external whenNotPaused {
        ...
        // Pull buyer's COMRADE
        require(comradeToken.transferFrom(msg.sender, l.seller, paid), "comrade to seller");
        if (fee > 0 && feeRecipient != address(0)) {
            require(comradeToken.transferFrom(msg.sender, feeRecipient, fee), "comrade fee");
        }
